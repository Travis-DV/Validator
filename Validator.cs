...
2. {message2}";
        ...
        public static Tuple<bool, int> CheckInt(string? input, int Min = 0)
        {
            int outputInt = 0;
            bool outputBool = true;


            if (input == null || !int.TryParse(input, out outputInt) || outputInt < Min)
            {
                outputBool = false;
            }
...
#if ConsoleApp
        public static int GetInt(string message, int Min = 0)
        {
            string? input = null;
            Tuple<bool, int> output = new Tuple<bool, int>(false, -1);

            string localMessage = $"{message} (int): ";
            ...
            while (!output.Item1)
            {
                ...
                output = Validator.CheckInt(input, Min: Min);
            }
